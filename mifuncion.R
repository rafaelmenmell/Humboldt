...
ExtraeObsMeteorologicasNOAA <- function(station_id,
                                 startyear,
                                 endyear,
                                 full_data = FALSE,
                                 add_data_report = FALSE,
                                 #select_additional_data = "GH1"){
                                 select_additional_data = c("AA1","GM1","GH1")){
  ...
  if (full_data == FALSE){
    ...
    # dplyr::filter data frame to only include data for requested years
  ...
  if (full_data == TRUE){
    ...
    # dplyr::filter those measured parameters and obtain string of identifiers
    significant_params <- data_categories[which(data_categories_counts >= 1)]
    
    # dplyr::filter the significantly available extra parameters by those specified
    ...
    if (data_categories[1] %in% significant_params){
      ...
      additional_data <-
        get_df_from_category(
          ...
          all_add_data)
      #comprobaciones
      if (length(additional_data[!is.na(additional_data$aa1_1) & additional_data$aa1_1==99,]$aa1_1)!=0){
        additional_data[!is.na(additional_data$aa1_1) & additional_data$aa1_1==99,]$aa1_1 <- NA
      }
      if (length(additional_data[!is.na(additional_data$aa1_2) & additional_data$aa1_2==999.9,]$aa1_2)!=0){
        additional_data[!is.na(additional_data$aa1_2) & additional_data$aa1_2==999.9,]$aa1_2 <- NA
      }
      additional_data <- additional_data[,1:2]
      colnames(additional_data) <- c("precip_period","precip")
    ...
    if (data_categories[43] %in% significant_params){
      ...
      large_data_frame$hrly_solar_rad <- additional_data
    }
...
ExtraeVariasEstaciones <- function(codigos,year1,year2,updateProgress = NULL){
  datos <- vector(mode="list",length=length(codigos))
  n <- 1
  for (n in 1:length(codigos)){
    datos[[n]] <- ExtraeObsMeteorologicasNOAA(station_id = codigos[n],startyear = year1,endyear = year2)
    updateProgress(detail=codigos[n])
  }
  datos <- bind_rows(datos)
  return(datos)
}

ExtraeRadiacionVariasEstaciones <- function(codigos,year1,year2,updateProgress = NULL){
  datos <- vector(mode="list",length=length(codigos))
  n <- 1
  for (n in 1:length(codigos)){
    station_id <-  codigos[n]
    aws <- sprintf("%06d",as.numeric(unlist(strsplit(station_id,"-"))[1]))
    if (unlist(strsplit(station_id,"-"))[2]=="99999"){
      wban <- "99999"
    } else {
      wban <- sprintf("%06d",as.numeric(unlist(strsplit(station_id,"-"))[2]))
    }
    GetSynopRadiacion(codigo_aws=aws,codigo_wban=wban,year1 =year1 ,year2 = year2)
    rad <- LeeRadiacion(codigo_aws=aws,codigo_wban=wban,year1 =year1 ,year2 = year2)
    datos[[n]] <- rad
    datos[[n]]$aws <- aws
    datos[[n]]$wban <- wban
    unlink(list.files(patter=station_id,full.names = TRUE))
  }
  datos <- bind_rows(datos)
  return(datos)
}